...
async def get_trainings(
    ...
    if len(trainings_list) == 0:
        return JSONResponse(
            ...
            content=f'Trainings not found with query params: {queries.dict(exclude_none=True)}',
        )
        
    request.app.logger.info(
        f'Return list of {len(trainings_list)} trainings,'
        + ' with query params:' + f'{queries.dict(exclude_none=True)}'
    )
    return trainings_list

@router.get(
    "/{training_id}", response_model=TrainingResponse,
